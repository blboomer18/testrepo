...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class', names='Launch Site',
                     title='Total Success Count by Launch Site')
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        fig = px.pie(filtered_df, names='class', title=f"Launch Success vs. Failure for {entered_site}")
    return fig
...
def get_scatter_chart(entered_site, payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                     title=f"Payload Mass vs Launch Success for {entered_site} (Class: 0 - Fail, 1 - Success)")
    return fig
